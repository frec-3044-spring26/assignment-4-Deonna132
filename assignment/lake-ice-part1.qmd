...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) + 
  ...
  geom_line() +
  labs(x = "Year",
       y = "Ice Off Day of the Year",
       title = "Ice Off Day over Time")
```
...
**Answer 2: Based on the plot, I would expect the slope to be negative (minus). I expect that because as the year increases, the ice off day of the year tends to decrease. So the ice is melting earlier in the year over time, meaning theres a negative relationship between year and ice off day**
...
**Answer 3: The direction is downward sloping, so negative, which matches my prediction of it having a negative relationship.**
...
**Answer 4: The R2 is 0.2055 is a low R2, meaning that only 20.55% explains the variation in ice off doy. So no, they are not strongly related.**
...
**Answer 6:The residuals are not normally distributed. The residuals are skewed left, so they do not meet the assumptions of the linear regression.**
...
**Answer 8: It would take about 66 years (7days/0.10602d ays a year). It is meaningful because while 0.1 day per year seems like a small change, over a vast amount of time like a decade, the results are means ecological shifts in seasonal timing**
...
**Answer 9: I feel pretty comfortable with it considering in my Intro to EDS class, we used linear regression models pretty frequently. Also, getting a refresher on what I can do with the lm model, like adding residuals and predictions only solidifes my previous comfortabliy and confidence with linear models.**
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  labs(x = "Year", 
       y = "Ice Off Day of the Year", 
       title = "Ice off Day of the Year over Time")
...
**Answer 11: The predicted ice off day in 2040 should be day 103 which is early April. The confidence interval is between 116.8214 and 122.0973. The confidence interval is 5.2759 or 2 days wide. **

```{r}
318.94856 + (-0.10602) * (2040)

  quantile(sunapee$residual, c(0.05, 0.95), na.rm = TRUE)
...
**Answer 12: I would say that the ice off date is changing more quickly in recent years but it had more variability. If you look at the plot of the earlier years, there is a downward trend and slow change over many decades and the spread is relatively tight. In recent years, there is a bigger vertical spread and a visually steeper drop. So melts are happening earlier than before, variability is higher and the decline looks steeper. There are earlier minimums in recent years and a more rapid drop over fewer years.**
```{r}
sunapee_early <- sunapee |>
  filter(year <= 1969)

sunapee_later <- sunapee |>
  filter(year >= 1970)
ggplot(sunapee_early, aes(x = year, y = ice_off_doy)) + geom_point()
hist(x = sunapee_early$residual)

ggplot(sunapee_later, aes(x = year, y = ice_off_doy)) + geom_point()
hist(x = sunapee_later$residual)
